...
convert_fn({ID, Blocks}, SSA = #ssa{nodes=Nodes}) when is_integer(ID) ->
    ...
    case Free of
        none ->
            ...
            Context = mmb_compute:init(),
            {_, Queue1, Context3} = create_vars(Params, Entry, Queue, Context, Nodes),
            SSA1 = SSA;
        _ ->
            {_, Queue, Context} = create_vars(Free, Entry, mmb_queue:init(), mmb_compute:init(), Nodes),
            ...
            {FnID, Context2} = mmb_compute:add_value({const, FnType, {fn, ID}}, Context1),
            Context3 = mmb_compute:bind_value({eval, FnType, {op, {load, tag}, [Closure]}}, FnID, Context2)
    end,
...
collect_value({'let', Var}, BlockID, Nodes, Locals, Queue, Context) ->
    ...
    Locals1 =
        case mmb_compute:eval(Type, Expr, Nodes, Context) of
            none ->
                {ValueID, Context1} = mmb_compute:add_value({var, Type, Var}, Context),
                Queue2 = mmb_queue:push({avail, BlockID, ValueID}, Queue),
                Locals#{ValueID => []};
            {ValueID, Context1} ->
                case mmb_compute:is_eval(ValueID, Context1) of
                    false ->
                        Queue2 = Queue,
                        Locals;
                    true ->
                        List = mmb_compute:uses(ValueID, Context1),
    ...
    Context3 = mmb_compute:set_var(Var, ValueID, Context1),
...
create_var(Var, BlockID, Queue, Context, Nodes) ->
    ...
    {ValueID,
     ...
     mmb_compute:set_var(Var, ValueID, Context1)}.
...
resolve_block_phi(Src, Acc, Input, Phis, Nodes, Context) ->
    #{Src := List} = Phis,
    {List1, Context1}= mmb_compute:resolve_vars(List, Nodes, Context),
...
phi_back(BlockID, ValueID, Phis, Context) ->
    ...
    {ValueID1, Phi1, Context1} = mmb_compute:phi_back(ValueID, Phi, Context),
...
rebuild_block(BlockID, ValuePhis, BlockMap, Context, ValueMap, Done, SSA = #ssa{nodes=Nodes}) ->
    ...
    Avails3 =
        case Srcs of
            [] ->
                [X || X <- Avails2, not mmb_compute:is_var(X, Context)];
            _ ->
                Avails2
        end,
...
rebuild_stmts([{'let', Var}|T], Locals, Context, ValueMap, SSA = #ssa{nodes=Nodes}) ->
    ValueID = mmb_compute:get_var(Var, Context),
    case ValueMap of
        #{ValueID := _} ->
            rebuild_stmts(T, Locals, Context, ValueMap, SSA);
        _ ->
            case mmb_compute:get_value(ValueID, Context) of
                {const, _, _} ->
                    rebuild_stmts(T, Locals, Context, ValueMap, SSA);
                _ ->
                    #{Var := {var, Type, Expr}} = Nodes,
...
rebuild_value(ValueID, Context, ValueMap, SSA) ->
    {eval, Type, Expr} = mmb_compute:get_value(ValueID, Context),
...
rebuild_expr({call, Fun, Args}, Context, ValueMap, SSA) ->
    {[Fun1|Args1], ValueMap1, SSA1} = rebuild_ref_list([Fun|Args], Context, ValueMap, SSA),
    {{call, Fun1, Args1}, ValueMap1, SSA1}.
...
tsort([H|T], Done, All, Context) ->
    case Done of
        ...
        _ ->
            List = mmb_compute:uses(H, Context),
...
add_value_phi(ValueID, Context, ValueMap, SSA) ->
    Type = mmb_compute:get_type(ValueID, Context),
...
bind_var(Var, Context, Nodes, ValueMap) ->
    #{Var := {var, Type, _}} = Nodes,
    ID = mmb_compute:get_value_id({var, Type, Var}, Context),
    ValueMap#{ID => Var}.
