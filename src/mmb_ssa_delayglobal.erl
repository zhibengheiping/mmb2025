...
convert_fn({ID, [Entry|Blocks]}, SSA = #ssa{nodes=Nodes}) when is_integer(ID) ->
    ...
    {Stmts1, Globals} = collect_globals(Stmts, #{}, Nodes),
    if map_size(Globals) =:= 0 ->
       ...
       true ->
            ...
            Stmts2 = append([{'let', X} || X <- maps:keys(Uses2)], Stmts1),
...
collect_globals([{'let', ID}|T]=Stmts, Acc, Nodes) ->
    #{ID := Node} = Nodes,
    case Node of
        {var, _, {op, {load, global, _}, []}} ->
            collect_globals(T, Acc#{ID => Node}, Nodes);
        _ ->
            {Stmts, Acc}
    end;
collect_globals(Stmts, Acc, _Nodes) ->
    {Stmts, Acc}.
